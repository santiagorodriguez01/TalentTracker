...
import { normalizeGenero } from "../../utils/normalizers.js";
...
async function replacePersonaRoles(personaId, roles, conn) {
  ...
  try {
    await connection.execute("DELETE FROM persona_rol WHERE persona_id=?", [
      personaId,
    ]);
    ...
    await connection.execute("UPDATE persona SET rol=? WHERE id=?", [
      primary,
      personaId,
    ]);
...
export const getById = asyncH(async (req, res) => {
  ...
  const rows = await query(
    "SELECT id, nombre, apellido, genero, dni, fecha_nac, email, telefono, domicilio, foto, qr_url, qr_ver, rol, estado FROM persona WHERE id=?",
    [id]
  );

  if (!rows.length)
    return res.status(404).json({ message: "No existe persona" });
...
export const create = asyncH(async (req, res) => {
  ...
  const initialRoles = Array.isArray(req.body?.roles)
    ? req.body.roles
    : req.body?.rol
    ? [req.body.rol]
    : [];
  ...
  try {
    ...
    const generoDB = normalizeGenero(genero);
    ...
    await audit(req.user?.id, "CREAR", "persona", personaId, {
      nombre,
      apellido,
      dni,
      roles,
...
export const update = asyncH(async (req, res) => {
  ...
  const normalizedRoles =
    rolesPayload != null ? normalizeRoles(rolesPayload) : null;
  ...
  for (const field of allowed) {
    if (Object.prototype.hasOwnProperty.call(body, field)) {
      ...
      if (field === "genero") {
        params.push(normalizeGenero(body.genero));
      } else {
  ...
  try {
    ...
    if (sets.length) {
      ...
      await conn.execute(
        `UPDATE persona SET ${sets.join(", ")} WHERE id=?`,
        params
      );
    }
    ...
    if (normalizedRoles !== null) {
      const { roles } = await replacePersonaRoles(
        personaId,
        normalizedRoles,
        conn
      );
    ...
    await audit(req.user?.id, "ACTUALIZAR", "persona", personaId, {
      cambiado: changed,
    });
...
export async function getPersonaById(req, res) {
  const rows = await query("SELECT * FROM persona WHERE id=?", [req.params.id]);
  if (!rows.length)
    return res.status(404).json({ message: "No existe persona" });
  res.json(rows[0]);
}
...
export async function generarQrPersona(req, res) {
  ...
  await query("UPDATE persona SET qr_url=? WHERE id=?", [qrUrl, id]);

  res.json({ qr_url: qrUrl });
}

export async function getQrPersona(req, res) {
  const rows = await query("SELECT qr_url FROM persona WHERE id=?", [
    req.params.id,
  ]);
  if (!rows.length)
    return res.status(404).json({ message: "No existe persona" });
...
export async function getPublicPersona(req, res) {
  ...
  res.json({
    ...
    socio: socio
      ? {
          estado_cuenta: socio.estado_cuenta,

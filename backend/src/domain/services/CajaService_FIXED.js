// ARCHIVO CORREGIDO - Reemplazar CajaService.js con este contenido
// CAMBIO PRINCIPAL: obtenerCajeros() ahora usa persona_rol en lugar de JOIN con caja
...
// ============================================
// FUNCIÓN CORREGIDA: obtenerCajeros()
// ============================================
export async function obtenerCajeros() {
  ...
  // Si no hay cajeros en persona_rol, usar el método antiguo (cajeros con movimientos)
  if (cajeros.length === 0) {
    return await query(`
      SELECT p.id, p.nombre, p.apellido, p.dni
      FROM caja c
